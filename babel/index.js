...
var _competingCountries = _interopRequireDefault(require("./types/competingCountries"));
...
class OlympicGames {
  constructor() {
    this.hooks = {
      admission: new _tapable.SyncHook(['countries']),
      openingCeremony: new _tapable.SyncBailHook(['program']),
      process: new _tapable.AsyncParallelHook()
    };
  } // 入场
...
tokyoOlympic.report('process'); // 2. 在钩子函数上挂载plugin（在入场时 设立检查环节，检查参赛国身份）

tokyoOlympic.hooks.admission.tap('sign', (countries = []) => {
  for (let i = 0; i < countries.length; i++) {
    const country = countries[i];

    if (_competingCountries.default.includes(country)) {
